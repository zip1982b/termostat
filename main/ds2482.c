	...
	*/
	...
	switch(ret){
		case ESP_OK:
			...
			{
				//handle error
				printf("[DS2482_write_config()] - tmp = %d not= read_config = %d and DS2482_reset()\n", tmp, read_config);
				DS2482_reset();
				return 0;
			}
...
uint8_t OWReset(void)
{
	...
	*/
	...
	{
		printf("[OWReset()] POLL_LIMIT, DS2482_reset()\n");
		DS2482_reset();
		return 0;
	}
	...
	*/
	...
	switch(ret){
		...
		case ESP_ERR_INVALID_ARG:
			...
			break;
		case ESP_FAIL:
			printf("[OWWriteByte()] - Sending command error, slave doesn`t ACK the transfer 1\n");
			break;
		case ESP_ERR_INVALID_STATE:
	...
	if(poll_count >= POLL_LIMIT)
	{
		printf("[OWWriteByte()] POLL_LIMIT, DS2482_reset()\n");
		DS2482_reset();
	}
}
...
//{ 
	...
	*/
	...
	switch(ret){
		case ESP_OK:
			printf("[OWReadByte()] - CMD_1WWB = OK \n");
			break;
		case ESP_ERR_INVALID_ARG:
			printf("[OWReadByte()] - Parameter error (1) \n");
			break;
		case ESP_FAIL:
			printf("[OWReadByte()] - Sending command error, slave doesn`t ACK the transfer 1\n");
			break;
		case ESP_ERR_INVALID_STATE:
			printf("[OWReadByte()] - i2c driver not installed or not in master mode \n");
			break;
	...
	if(poll_count >= POLL_LIMIT)
	{
		printf("[OWReadByte()] POLL_LIMIT, DS2482_reset() exit\n");
		DS2482_reset();
		return 0;
	}
	i2c_master_start(cmd);  //S
...
uint8_t DS2482_search_triplet(uint8_t search_direction)
{
	...
	*/
	...
	while ((status & STATUS_1WB) && (poll_count++ < POLL_LIMIT)); //Repeat until 1WB bit has changed to 0  --  && (poll_count++ < POLL_LIMIT)
