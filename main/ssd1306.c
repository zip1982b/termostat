...
void vDrawMenu(uint8_t status_relay, float temperature)
{
	/*** Process view ***/
		...
		SSD1306_Puts("Status", &Font_7x10, SSD1306_COLOR_WHITE); // шрифт Font_7x10, белым цветом
		SSD1306_DrawLine(10, 12, 110, 12, SSD1306_COLOR_WHITE); // draw line
		SSD1306_GotoXY(5, 15);
		if(status_relay)
			SSD1306_Puts("Pump ON", &Font_7x10, SSD1306_COLOR_WHITE);
		else
			SSD1306_Puts("Pump OFF", &Font_7x10, SSD1306_COLOR_WHITE);
		SSD1306_GotoXY(5, 35);
		SSD1306_Puts("t=", &Font_7x10, SSD1306_COLOR_WHITE);
		
		char buf[18];
		sprintf(buf, "%.3f", temperature);
